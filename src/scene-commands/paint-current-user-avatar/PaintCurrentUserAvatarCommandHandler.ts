	...
	implements CommandHandler<PaintCurrentUserAvatarCommand>
{
	...
	private async createAvatarBadge(imageBuffer: ArrayBuffer, userName: string): Promise<void> {
	...
	private createAvatarImage(avatarImage: ArrayBuffer, currentUserName: string): EllipseNode {
		...
		imageWrapper.fills = [{ type: "IMAGE", scaleMode: "FILL", imageHash: figmaImage.hash }];
	...
	private async createAvatarText(userName: string): Promise<TextNode> {
		...
		userNameText.y =
			this.figma.viewport.center.y + this.avatarImageSize + this.avatarImageSize / 12;

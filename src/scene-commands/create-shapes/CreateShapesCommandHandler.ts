...
export class CreateShapesCommandHandler implements CommandHandler<CreateShapesCommand> {
	private readonly separationBetweenShapes = 150;

	constructor(private readonly figma: PluginAPI) {}

	handle({ payload: { numberOfShapes, typeOfShapes } }: CreateShapesCommand): void {
		const shapes = Array.from({ length: numberOfShapes }).map(this.toShape(typeOfShapes));

		this.focusUiOn(shapes);
	}

	private toShape(typeOfShapes?: SupportedShapes): (_: unknown, iteration: number) => SceneNode {
	...
	private createShape(typeOfShapes: SupportedShapes): () => RectangleNode | EllipseNode {
		return (): RectangleNode | EllipseNode =>
			typeOfShapes === "Rectangle" ? this.figma.createRectangle() : this.figma.createEllipse();
	}
	...
	private styleShape(shape: RectangleNode | EllipseNode, shapeNumber: number): void {
